 ...
 *  Copyright (c) 2020 Lee Dongjin <dongjin@apache.org>
 */
...
static unsigned int rightcmd_as_rightctrl = 0;
module_param(rightcmd_as_rightctrl, uint, 0644);
MODULE_PARM_DESC(rightcmd_as_rightctrl, "Use the right Command key as a right Ctrl key. "
		"[0] = as-is, Mac layout. 1 = Right Command is right Ctrl");
...
static const struct apple_key_translation rightcmd_as_rightctrl_keys[] = {
		...
		struct hid_usage *usage, __s32 value)
{
	...
	if (rightcmd_as_rightctrl) {
		trans = apple_find_translation(rightcmd_as_rightctrl_keys, usage->code);
		if (trans) {
...
static void apple_setup_input(struct input_dev *input)
{
	...
	if (ejectcd_as_delete) {
        ...
        if (rightcmd_as_rightctrl) {
		for (trans = rightcmd_as_rightctrl_keys; trans->from; trans++)
			set_bit(trans->to, input->keybit);
	}
}
